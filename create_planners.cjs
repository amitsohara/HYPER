...
const writePlanner = (name, className, explain, failUnlessKnowledge = false) => {
    const code = `import { IPlanningStrategy } from "../IPlanningStrategy.js";
import { PlanObject, GoalObject, PlanStatus, AtomicTask, TaskStatus } from "../types.js";
import { v4 as uuidv4 } from "uuid";

export class ${className} implements IPlanningStrategy {
    getName(): string {
        return "${name}";
    }
    ...
    async generatePlans(goal: GoalObject, context: any): Promise<PlanObject[]> {
        ...
        ${failUnlessKnowledge ? `if (!knowledge) return [];` : `if (goal.name.includes("fallback_trigger") && !knowledge) return [];`}
...
`;
    fs.writeFileSync(`src/server/core/hpe1/planners/${className}.ts`, code);
};

writePlanner('GOAP_PLANNER', 'GOAPPlanner', 'Generated via Goal-Oriented Action Planning.');
writePlanner('GRAPH_PLANNER', 'GraphPlanner', 'Generated via Graph-based heuristic search.');
writePlanner('CONSTRAINT_PLANNER', 'ConstraintPlanner', 'Generated via Constraint Satisfaction.');
writePlanner('UTILITY_PLANNER', 'UtilityPlanner', 'Generated via Expected Utility Maximization.');

const htnCode = `import { IPlanningStrategy } from "../IPlanningStrategy.js";
import { PlanObject, GoalObject, PlanStatus, AtomicTask, TaskStatus } from "../types.js";
import { v4 as uuidv4 } from "uuid";
...
`;
fs.writeFileSync('src/server/core/hpe1/planners/HTNPlanner.ts', htnCode);
