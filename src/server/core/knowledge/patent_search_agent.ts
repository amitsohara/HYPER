...
export async function patentSearch(ai: GoogleGenAI, query: string) {
    const prompt = `Use the Google Search tool to perform a real, up-to-date web search specifically focusing on patents and intellectual property for: "${query}".
...
[
  {
    ...
    try {
        ...
        console.warn('patentSearch error:', e);
        return [];
    }
}
