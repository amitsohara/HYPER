...
export async function researchSearch(ai: GoogleGenAI, query: string) {
    const prompt = `Use the Google Search tool to perform a real, up-to-date web search specifically focusing on research papers and academic publications for: "${query}".
...
[
  {
    ...
    try {
        ...
        console.warn('researchSearch error:', e);
        return [];
    }
}
