...
export async function newsSearch(ai: GoogleGenAI, query: string) {
    const prompt = `Use the Google Search tool to perform a real, up-to-date web search specifically focusing on recent news articles and current events for: "${query}".
...
[
  {
    ...
    try {
        ...
        console.warn('newsSearch error:', e);
        return [];
    }
}
