...
export async function governmentSearch(ai: GoogleGenAI, query: string) {
    const prompt = `Use the Google Search tool to perform a real, up-to-date web search specifically focusing on government data, statistics, and official reports for: "${query}".
...
[
  {
    ...
    try {
        ...
        console.warn('governmentSearch error:', e);
        return [];
    }
}
