export class DomainVocabularyAnalyzer {
  static analyze(text: string, profile: any): { score: number; violations: string[] } {
    ...
    if (!profile.primary_domain.toLowerCase().includes("business") && 
        !profile.primary_domain.toLowerCase().includes("startup") && 
        !profile.primary_domain.toLowerCase().includes("enterprise")) {
