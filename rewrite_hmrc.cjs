...
export class MissionResultManager {
    ...
    private handleAnyEvent(event: any) {
        ...
        switch (event.type) {
            case "MISSION_CREATED":
                ctx.mission = payload.mission || payload;
                break;
    ...
    private finalizeMission(event: any, missionId: string) {
        ...
        if (latestReasoning) {
            cognitiveReasoningSummary = latestReasoning.summary || latestReasoning.content || "";
        }
        
        // Extract recommendations from plans
        ...
        if (rootCauses.length === 0) {
            for (const r of ctx.reasoning) {
                if (r.explanation?.reasoningTrace) rootCauses.push(...r.explanation.reasoningTrace);
            }
        }
...
console.log("Written MissionResultManager.ts");
